...
namespace App\Doctrine\Command\Tools;
...
class SchemaTool
{
    ...
    private function addDiscriminatorColumnDefinition(ClassMetadata $class, Table $table): void
    {
        ...
        if (
            ...
            ('string' === mb_strtolower($discrColumn['type']) && !isset($discrColumn['length']))
        ) {
    ...
    private function gatherColumn(
        ...
        // todo: Вариант рабочий, но принят на скорую руку... Оптимален?
        if (isset($mapping['options']) && isset($mapping['options']['virtual'])) {
            return;
        }
        ...
        if ('string' === mb_strtolower((string) $columnType) && null === $options['length']) {
            $options['length'] = 255;
        }
    ...
    private function gatherRelationsSql(
        ...
        foreach ($class->associationMappings as $id => $mapping) {
            ...
            } elseif (ClassMetadata::ONE_TO_MANY === $mapping['type'] && $mapping['isOwningSide']) {
                // ... create join table, one-many through join table supported later
                throw NotSupported::create();
            } elseif (ClassMetadata::MANY_TO_MANY === $mapping['type'] && $mapping['isOwningSide']) {
                // create join table
    ...
    private function gatherRelationJoinColumns(
        ...
        foreach ($joinColumns as $joinColumn) {
            ...
            if (!$theJoinTable->hasColumn($quotedColumnName)) {
                ...
                if ('string' === $fieldMapping['type'] && isset($fieldMapping['length'])) {
                    $columnOptions['length'] = $fieldMapping['length'];
                } elseif ('decimal' === $fieldMapping['type']) {
